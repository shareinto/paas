...
test('应用详情展示精简入口和 Workload 列表', async () => {
  renderPage();

  expect(await screen.findByRole('tab', { name: '镜像构建' })).toBeInTheDocument();
  expect(screen.getByRole('tab', { name: '工作负载' })).toBeInTheDocument();
  ...
  await userEvent.click(screen.getByRole('tab', { name: '工作负载' }));
