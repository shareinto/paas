...
test('Freight 按创建时间从左到右按新到旧显示并展示流程引导', async () => {
  renderPromotionPage();
...
test('Freight 卡片审批按钮打开 Freight 审批弹窗', async () => {
  ...
  expect(within(dialog).getByText('20260611.1')).toBeInTheDocument();
...
test('创建 Freight 抽屉使用表格列出所有启用 Workload 并按行校验', async () => {
  ...
  const nameInput = within(drawer).getByPlaceholderText('请输入 Freight 名称');
  await userEvent.clear(nameInput);
  await userEvent.type(nameInput, '20260612.1');
  await userEvent.click(within(drawer).getByRole('button', { name: '创建 Freight' }));

  await waitFor(() => {
    const timeline = screen.getByLabelText('Freight 时间轴');
    const [firstCard] = within(timeline).getAllByTestId('freight-card');
    expect(within(firstCard).getByTestId('freight-name')).toHaveTextContent('20260612.1');
  });
});
