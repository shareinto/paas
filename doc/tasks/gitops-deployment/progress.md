...
- 已实现 MySQL 正式表仓储、部署模板版本、应用模板从平台基础模板复制、模板校验、values.yaml 更新、所有 Stage 直接 commit、ManifestRevision 记录、Deployment 查询、Agent 状态映射和回滚 digest 场景测试。
- 已补齐平台基础模板、应用模板创建/查询/更新 API；模板变更、ManifestRevision 和 Deployment 创建会记录审计事件。staging/prod 等高风险环境的准入由 PaaS Promotion 审批流控制。
