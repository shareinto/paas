...
test('真实 API 列表无 items 时点击 Freight 会加载详情并展示 Workload 镜像', async () => {
  vi.stubEnv('VITE_API_BASE_URL', 'https://paas.example');
	  const fetchMock = vi.fn(async (url: string) => {
        ...
        stage_eligibility: { dev: ['freight_1'] },
      }), { status: 200 });
    }
    if (url.endsWith('/api/apps/app_1/delivery/stages/dev/eligible-freights')) {
...
test('真实 API 使用当前路由应用 ID 和稳定 Stage key 发布', async () => {
  vi.stubEnv('VITE_API_BASE_URL', 'https://paas.example');
	  const fetchMock = vi.fn(async (url: string, options?: RequestInit) => {
        ...
        latest_artifacts_by_workload: {},
        stage_eligibility: { dev: ['freight_other'] },
      }), { status: 200 });
    }
    if (url.endsWith('/api/apps/app_other/delivery/stages/dev/eligible-freights')) {
    ...
    if (url.endsWith('/api/apps/app_other/delivery/stages/dev/promotions') && options?.method === 'POST') {
      const body = JSON.parse(String(options.body || '{}'));
      expect(body).toMatchObject({
        freight_id: 'freight_other',
        target_stage_key: 'dev'
      });
      expect(body.target_cluster_ids).toEqual([]);
      expect(body.namespace_override).toBe('');
  ...
  expect(fetchMock).toHaveBeenCalledWith('https://paas.example/api/apps/app_other/delivery/stages/dev/eligible-freights', expect.any(Object));
  expect(fetchMock).toHaveBeenCalledWith('https://paas.example/api/apps/app_other/delivery/stages/dev/promotions', expect.objectContaining({ method: 'POST' }));
