...
test('应用详情中创建流水线选择一个运行时环境并添加代码源', async () => {
  window.localStorage.setItem('paas_token', 'test');
  useSession.setState({ token: 'test', userName: '测试用户' });
  renderFlow('/apps/app_1/build');
  await userEvent.click(await screen.findByRole('button', { name: /创建流水线/ }));
  const dialog = await screen.findByRole('dialog', { name: '创建构建流水线' });
  expect(dialog.querySelector('.pipeline-editor-form')).toHaveClass('ant-form-horizontal');
  expect(dialog.querySelector('.pipeline-editor-form')).not.toHaveClass('ant-form-vertical');
  expect(dialog.querySelector('.pipeline-form-stack')).toBeInTheDocument();
  expect(dialog.querySelector('.pipeline-form-grid')).not.toBeInTheDocument();
  expect(within(dialog).getByText('基本信息')).toBeInTheDocument();
  expect(within(dialog).getByText('代码源')).toBeInTheDocument();
  ...
  expect(within(dialog).getAllByText('springboot-jdk11').length).toBeGreaterThan(0);
...
test('应用详情中创建流水线后列表展示新流水线', async () => {
  ...
  const displayName = within(editDialog).getByDisplayValue('流水线 2');
...
test('构建管理支持编辑构建环境和运行时环境', async () => {
  ...
  expect(await screen.findByText('springboot-jdk11')).toBeInTheDocument();
  ...
  expect(within(runtimeDialog).getByDisplayValue('springboot-jdk11')).toBeDisabled();
  expect(within(runtimeDialog).getByDisplayValue('阿里云 Dragonwell')).toBeInTheDocument();
...
test('核心交付路径包含构建日志、版本、部署画布和审计', async () => {
  window.localStorage.setItem('paas_token', 'test');
  useSession.setState({ token: 'test', userName: '测试用户' });
  renderFlow('/apps/app_1/build');
  const pipelinePanel = await screen.findByTestId('pipeline-panel');
  ...
  cleanup();
  renderFlow('/freights');
  expect(await screen.findByRole('columnheader', { name: 'Freight' })).toBeInTheDocument();
...
test('核心控制台页面可以通过 mock API 独立渲染', async () => {
  ...
  cleanup();
  renderFlow('/apps/app_1/build');
  expect((await screen.findAllByLabelText('选择应用')).length).toBeGreaterThan(0);
  expect(screen.queryByRole('heading', { name: '订单服务' })).not.toBeInTheDocument();
  expect(screen.queryByText('应用标识')).not.toBeInTheDocument();
  expect(screen.queryByRole('tab', { name: '构建' })).not.toBeInTheDocument();
  expect(screen.queryByRole('heading', { name: '流水线' })).not.toBeInTheDocument();
...
test('项目详情作为工作台展示项目内应用和源码仓库', async () => {
  window.localStorage.setItem('paas_token', 'test');
  useSession.setState({ token: 'test', userName: '测试用户' });
  renderFlow('/projects/project_1');

  expect(await screen.findByRole('heading', { name: '订单平台' })).toBeInTheDocument();
  expect(await screen.findByRole('tab', { name: '应用' })).toBeInTheDocument();
  expect(screen.getByRole('tab', { name: '源码仓库' })).toBeInTheDocument();
  expect(await screen.findByText('订单服务')).toBeInTheDocument();
  expect(screen.getByText('订单前端')).toBeInTheDocument();

  await userEvent.click(screen.getByRole('tab', { name: '源码仓库' }));
  expect(await screen.findByText('订单服务仓库')).toBeInTheDocument();
  expect(screen.queryByPlaceholderText('所属项目')).not.toBeInTheDocument();
});

test('从项目工作台创建应用时固定当前项目上下文', async () => {
  window.localStorage.setItem('paas_token', 'test');
  useSession.setState({ token: 'test', userName: '测试用户' });
  renderFlow('/projects/project_1');

  await userEvent.click(await screen.findByRole('button', { name: /创建应用/ }));

  expect(await screen.findByRole('heading', { name: '创建应用' })).toBeInTheDocument();
  expect(await screen.findByText('订单平台')).toBeInTheDocument();
