...
test('真实 API 查询 Workload Stage 配置映射配置文件和可写目录真实字段', async () => {
  vi.stubEnv('VITE_API_BASE_URL', 'https://paas.example');
  vi.stubGlobal('fetch', vi.fn(async (url: string) => {
    if (url.endsWith('/api/applications/app_1/workloads/workload_api/stage-configs')) {
      return jsonResponse({
        items: [
          {
            id: 'config_prod',
  ...
  const api = await import('./index');
  await expect(api.listWorkloadStageConfigs('app_1', 'workload_api')).resolves.toMatchObject([
    {
      id: 'config_prod',
...
test('真实 API 保存 Workload Stage 配置', async () => {
  vi.stubEnv('VITE_API_BASE_URL', 'https://paas.example');
  let saveBody: Record<string, unknown> = {};
  vi.stubGlobal('fetch', vi.fn(async (url: string, init?: RequestInit) => {
    if (url.endsWith('/api/applications/app_1/workloads/workload_api/stage-configs/dev') && init?.method === 'PUT') {
  ...
  const api = await import('./index');
  await expect(api.saveWorkloadStageConfig('app_1', 'workload_api', 'dev', {
    replicas: 2,
