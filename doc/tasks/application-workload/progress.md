...
- 早期实现要求创建 Application 时绑定已有 `SourceRepository`；当前交互已调整为创建 BuildPipeline 时直接填写 Git/SVN 源码地址。
...
- 创建 Application 不再要求选择 `SourceRepository`，源码地址改由 BuildPipeline 代码源配置维护。
...
- 用户在应用详情中创建 BuildPipeline 时填写源码类型、源码地址、构建引用、source_path、构建环境、运行时环境和 BuildSpec；Workload 在创建或编辑时选择关联流水线。
