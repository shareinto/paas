...
test('真实 API 构建历史按开始时间倒序编号', async () => {
  ...
  const firstBuild = (await within(dialog).findByText('构建 1')).closest('button') as HTMLElement;
  const secondBuild = within(dialog).getByText('构建 2').closest('button') as HTMLElement;
