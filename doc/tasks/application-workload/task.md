...
- [x] 实现创建 Application 用例：创建应用基础信息，不创建应用级运行环境模型。
...
- [x] 提供 API：应用 CRUD、Workload CRUD、Workload 默认配置、Workload Stage 覆盖配置和应用 Stage 状态。
- [x] 编写测试：创建应用不生成应用级运行环境模型、ApplicationSource 兼容校验；新的 BuildPipeline 测试覆盖 SourceRepository、monorepo source_path 和 BuildSpec 校验。
