...
- 完成内容：`SourceRepository`、`RepositoryMigration`、`RepositoryPermissionSyncJob` 领域模型；内存仓储；MySQL 迁移草案；`GitSourceRepositoryPort`；服务层用例；HTTP API；Java 仓库扫描与 BuildSpec 建议生成。
...
- 2026-05-31 更新：新增 `internal/modules/sourcerepository/repository_mysql.go`，支持 `PAAS_REPOSITORY_DRIVER=mysql` 下的业务状态持久化。当前为第一阶段快照式 MySQL repository，后续可替换为逐表 SQL repository。
